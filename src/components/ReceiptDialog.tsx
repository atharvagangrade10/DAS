...
const ReceiptDialog: React.FC<ReceiptDialogProps> = ({ isOpen, onOpenChange, receiptData }) => {
  const handleDownloadPDF = () => {
    ...
    // Create a simple HTML content for the receipt
    const htmlContent = `
      <!DOCTYPE html>
      <html>
      <head>
        ...
        <style>
          ...
          .status-badge { display: inline-block; padding: 4px 8px; border-radius: 4px; background-color: #e6f3e6; color: #1f7a1f; font-weight: bold; }
        </style>
      </head>
      <body>
        <div class="header">
        ...
        <div class="section">
          ...
          <div><span class="label">Yatra Dates:</span> <span class="value">${format(new Date(receiptData.yatra_start_date), "MMM dd, yyyy")} to ${format(new Date(receiptData.yatra_end_date), "MMM dd, yyyy")}</span></div>
          <div><span class="label">Participant:</span> <span class="value">${receiptData.participant_name}</span></div>
          <div><span class="label">Participant ID:</span> <span class="value">${receiptData.participant_id}</span></div>
        </div>

        <div class="section">
          <div><span class="label">Payment Amount:</span> <span class="value">₹${receiptData.payment_amount}</span></div>
          <div><span class="label">Payment Status:</span> <span class="status-badge">${receiptData.payment_status}</span></div>
          <div><span class="label">Razorpay Order ID:</span> <span class="value">${receiptData.razorpay_order_id}</span></div>
          <div><span class="label">Razorpay Payment ID:</span> <span class="value">${receiptData.razorpay_payment_id}</span></div>
        </div>
    ...
    link.href = url;
    link.download = `receipt_${receiptData.yatra_name.replace(/\s+/g, '_')}_${receiptData.participant_id}.pdf`;
